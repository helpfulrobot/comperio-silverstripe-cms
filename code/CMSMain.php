...
class CMSMain extends LeftAndMain implements CurrentPageIdentifier, PermissionProvider
{
    ...
    public static $menu_title = 'Pages';
     ...
     */
    public static $site_tree_filter_options = array(
    ...
    public static function T_SiteTreeFilterOptions()
    {
        return array(
    ...
    public function getfilteredsubtree()
    {
        ...
        if (!isset($_REQUEST['filter'])) {
            die('No filter passed');
        }
        if (!ClassInfo::exists($_REQUEST['filter'])) {
            die('That filter class does not exist');
        }
        if (!is_subclass_of($_REQUEST['filter'], 'CMSSiteTreeFilter')) {
            die('That is not a valid filter');
        }
     ...
     */
    public function SiteTreeFilters()
    {
    ...
    public function generateDataTreeHints()
    {
        ...
        foreach ($classes as $class) {
            ...
            if ($allowedChildren != "none") {
                $def[$class]['allowedChildren'] = $allowedChildren;
            }
            ...
            if (is_array($allowedChildren)) {
                foreach ($allowedChildren as $allowedChild) {
...
JS;
     ...
     */
    public function ReportSelector()
    {
     ...
     * @return LiteralField
     */
    public function ReportFormParameters()
    {
        $forms = array();
        foreach ($this->SideReports() as $report) {
            if ($report->canView()) {
     ...
     */
    public function tellBrowserAboutPublicationChange($page, $statusMessage)
    {
    ...
    public function performRollback($id, $version)
    {
    ...
    public function sendFormToBrowser($templateData)
    {
        if (Director::is_ajax()) {
            SSViewer::setOption('rewriteHashlinks', false);
    ...
    public function canceldraftchangesdialog()
    {
        Requirements::clear();
    ...
    public function AddPageOptionsForm()
    {
        $pageTypes = array();

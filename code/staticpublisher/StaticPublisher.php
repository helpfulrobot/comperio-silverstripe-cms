...
abstract class StaticPublisher extends DataObjectDecorator
{
     ...
     */
    public static $echo_progress = false;
    ...
    * is saved, it is written to the cache 
     ...
     */
    public static $static_publisher_theme=false;
        ...
        foreach ($xlinks as $xlink => $assetsOnly) {
            ...
            if ($matches) {
                foreach ($matches as $item) {
                    $url = $item . '';
                    if ($assetsOnly && substr($url, 0, 7) != ASSETS_DIR . '/') {
                        continue;
                    }
    ...
    public function set_static_publisher_theme($theme)
    {
        self::$static_publisher_theme=$theme;
    }
    
    public function static_publisher_theme()
    {

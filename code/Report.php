...
class SS_Report extends ViewableData
{
    ...
    public function getCMSFields()
    {
        ...
        if ($params = $this->parameterFields()) {
            ...
            foreach ($params as $param) {
                if ($param instanceof HiddenField) {
                    $fields->push($param);
                } else {
                    $filters->push($param);
                }
    ...
    public function getReportField()
    {
        ...
        if ($fieldFormatting) {
            $tlf->setFieldFormatting($fieldFormatting);
        }
        if ($csvFieldFormatting) {
            $tlf->setCSVFieldFormatting($csvFieldFormatting);
        }
        if ($fieldCasting) {
            $tlf->setFieldCasting($fieldCasting);
        }
     ...
     */
    public static function register($list, $reportClass, $priority = 0)
    {
     ...
     */
    public static function unregister($list, $reportClass)
    {
     ...
     */
    public static function get_reports($list)
    {
        $output = array();
        if (isset(self::$registered_reports[$list])) {
            ...
            foreach ($sortedListItems as $report) {
                if (strpos($report, '(') === false) {
                    ...
                    $reportObj = eval("return new $report;");
                }
...
class SS_Report_FakeQuery extends SQLQuery
{
    ...
    public function __construct($obj, $method, $params)
    {
        $this->obj = $obj;
        $this->method = $method;
        $this->params = $params;
     ...
     */
    public function setSortColumnMethod($sortColMethod)
    {
